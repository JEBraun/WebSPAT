...
def initiationMsg():
    ...
    print('\n' + ('-'*termW))
    cprint(' '*centerTxt(msgStr)[0] + msgStr + ' '*centerTxt(msgStr)[1], 'green', attrs = ['bold'])
    print(('-'*termW)+ '\n')
    return(startTime)
...
def getSavePaths(stationInfo, vid):
    ...
    mod = ['os' , 're', 'opencv', 'math', 'json', 'time', 'termcolor', 
           'itertools', 'numpy', 'pandas', 'tkinter', 'noaa_coops', 
           'pathlib', 'termcolor', 'threading', 'PIL', 'matplotlib', 'skimage', 
